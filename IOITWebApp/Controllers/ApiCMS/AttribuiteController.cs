...
namespace IOITWebApp.Controllers.ApiCMS
{
    ...
    [ApiController]
    public class AttribuiteController : ControllerBase
    {
        ...
        public IActionResult GetByPage([FromQuery] FilteredPagination paging)
        {
            ...
            if (paging != null)
            {
                using (var db = new CNTTVNWebContext())
                {
                    ...
                    IQueryable<Attribuite> data = db.Attribuite.Where(c => c.Status != (int)Const.Status.DELETED);
                    ...
                    if (paging.page_size > 0)
                    {
                        ...
                        {
                            data = data.OrderBy("AttribuiteId desc").Skip((paging.page - 1) * paging.page_size).Take(paging.page_size);
                        }
                    }
                    else
                    {
                        ...
                        {
                            data = data.OrderBy("AttribuiteId desc");
                        }
                    }
        ...
        public async Task<IActionResult> GetAttribuite(int id)
        {
            ...
            try
            {
                using (var db = new CNTTVNWebContext())
                {
                    Attribuite data = await db.Attribuite.FindAsync(id);
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> PutAttribuite(int id, Attribuite data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
                ...
                using (var db = new CNTTVNWebContext())
                {
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        catch (DbUpdateException e)
                        {
                            ...
                            if (!AttribuiteExists(data.AttribuiteId))
                            {
                                def.meta = new Meta(404, "Not Found");
                                return Ok(def);
                            }
        ...
        // POST: api/Attribuite
        [HttpPost]
        public async Task<IActionResult> PostAttribuite(Attribuite data)
        {
            DefaultResponse def = new DefaultResponse();
            //check role
            var identity = (ClaimsIdentity)User.Identity;
            ...
            try
            {
                using (var db = new CNTTVNWebContext())
                {
                    ...
                    using (var transaction = db.Database.BeginTransaction())
                    {
                        ...
                        List<ProductAttribuite> productAttribuites = db.ProductAttribuite.Where(pa => pa.AttribuiteId == data.AttribuiteId && pa.Status != (int)Const.Status.DELETED).ToList();
        ...
        private bool AttribuiteExists(int id)
        {
            using (var db = new CNTTVNWebContext())
            {
                return db.Attribuite.Count(e => e.AttribuiteId == id) > 0;
            }
        }
    }
}