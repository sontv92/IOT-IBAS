...
import { User, NHANVIEN } from '../../data/Model';
...
export class NhanVienComponent implements OnInit {
  ...
  GetListNhanVien() {
    ...
    // }
    this.http.get('/api/NhanVien/GetByPage?page=' + this.paging.page + '&page_size=' + this.paging.page_size + '&query=' + this.paging.query + '&order_by=' + this.paging.order_by+ '&companyid=' + this.paging.CompanyId + '&Branchlist=' + this.paging.Branchid, this.httpOptions).subscribe(
      (res) => {
  ...
  AddUserFunc() {
    ...
    if (this.Item.ID) {
      this.Item.BranchId = this.q.BranchId;
      this.http.put('/api/NhanVien/' + this.Item.ID, this.Item, this.httpOptions).subscribe(
        (res) => {
          if (res["meta"]["error_code"] == 200) {
            this.GetListNhanVien();
            this.userModal.hide();
      ...
      // }
      this.Item.BranchId = this.q.BranchId;
      this.http.post('/api/NhanVien', this.Item, this.httpOptions).subscribe(
        (res) => {
          if (res["meta"]["error_code"] == 200) {
            this.GetListNhanVien();
            this.userModal.hide();
  ...
  XoaKH(Id,BranchId) {
    ...
    this.http.delete('/api/NhanVien/' + strID, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.GetListNhanVien();
          this.viewRef.clear();
