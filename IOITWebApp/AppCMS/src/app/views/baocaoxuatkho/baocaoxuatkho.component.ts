...
@Component({
  selector: 'app-baocaoxuatkho',
  templateUrl: './baocaoxuatkho.component.html',
...
export class BaoCaoXuatKhoComponent implements OnInit {
  ...
  public listBaoCaoXuatKho = [];
  public role: number;
  public disable: boolean = true;
  public loading: boolean;
  public listColumnName = [];
  public countListColumnName: number;
  public disableDenNgay: boolean = true;
  public listGroup = [
    { value: "KH", name: "Người nhận hàng" },
    { value: "NT", name: "Ngày trộn" },
    { value: "SP", name: "Mã sản phẩm" }];
  public listMaLienKetKH = [];
  public listMaLienKetMac = [];
  public listCuaVL = [];
  companyId: number;
  BranchId: string;
  tongKg: string;
  ...
  getListMaLienKetKH(val) {
    this.http.get('/api/baocaoxuatkho/getmalienketkh/' + val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listMaLienKetKH = res["data"];
          console.log('this.listMaLienKetKH', this.listMaLienKetKH);
        }
      },
      (err) => {
  ...
  getListMaLienKetMAC(val) {
    this.http.get('/api/baocaoxuatkho/getmalienketmac/' + val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listMaLienKetMac = res["data"];
          console.log('this.listMaLienKetMac', this.listMaLienKetMac);
        }
      },
      (err) => {
        console.log("Error: connect to API");
  ...
  getListCuaVL(val) {
    this.http.get('/api/cuavl/gettencuavlactive/' + val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listCuaVL = res["data"];
          console.log('this.listCuaVL', this.listCuaVL);
        }
      },
      (err) => {
        console.log("Error: connect to API");
  ...
  changeFn(val) {
    ...
    this.GetlistCD();
    this.getListCuaVL(val);
    this.getListMaLienKetKH(val);
    this.getListMaLienKetMAC(val);
  }
  ...
  GetListSlide() {
    ...
    this.listBaoCaoXuatKho = [];

    if (this.paging.CompanyId == undefined) {
      this.paging.CompanyId = 0;
    }
    this.http.get('/api/baocaoxuatkho/GetByPage?page=' + this.paging.page
      ...
      this.httpOptions).subscribe(
        (res) => {
          if (res["meta"]["error_code"] == 200) {
            if (res["data"] != null) {
              this.listBaoCaoXuatKho = res["data"];
              this.paging.item_count = res["metadata"];

              switch (this.q.GroupBy) {
                case "KH":
                  this.listBaoCaoXuatKho.forEach(item => {
                    item.Key = "Người nhận: " + item.Key;
                  });
                  break;
                case "NT":
                  this.listBaoCaoXuatKho.forEach(item => {
                    item.Key = "Ngày trộn: " + item.Key;
                  });
                  break;
                case "SP":
                  this.listBaoCaoXuatKho.forEach(item => {
                    item.Key = "Mã sản phẩm: " + item.Key;
                  });
                  break;
                default:
                  this.listBaoCaoXuatKho.forEach(item => {
  ...
  ExportExcel() {
    ...
    //  debugger;
    fetch('/api/baocaoxuatkho/GetReportChiTiet?page=' + this.paging.page + '&page_size=' + this.paging.page_size + '&order_by=' + this.paging.order_by
      + '&tungay=' + _tungay
      + '&denngay=' + _denngay
      + '&timetungay=' + this.q.timetungay
      + '&timedenngay=' + this.q.timedenngay
      ...
      .then(function (response) {
        ...
        this.DownloadFile(blob, this.commonService.ConvertUrl(tenchinhanh.Name) + "_bao_cao_xuat_kho_" + date + ".xlsx", 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  ...
  changeTuNgay() {
    this.q.denngay = this.q.tungay;
  }
  // Get danh sách khách hàng
  GetlistKH(val) {
    this.http.get('/api/thongkechitietvattu/GetKH/' + val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
  ...
  GetlistTenMacBeTong(val) {
    this.http.get('/api/thongkechitietvattu/GetTenMacBeTong/' + val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
  ...
  GetListVatLieu() {
    this.http.get('/api/thongkechitietvattu/GetVatLieu?companyid=' + this.paging.CompanyId + '&Branchlist=' + this.paging.Branchid, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
