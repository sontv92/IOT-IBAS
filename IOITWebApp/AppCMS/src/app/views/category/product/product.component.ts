...
export class CateProductComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscription = this.callCategoryFunctionService.getAction().subscribe(action => {
      if (action.TypeAction == 1) {
        this.OpenCateProductModal(undefined, action.CategoryId);
      } else if (action.TypeAction == 2) {
        this.OpenCateProductModal(action.CategoryId, undefined);
  ...
  //Mở modal thêm mới
  OpenCateProductModal(CategoryId, CategoryParentId) {
    this.Item = new Category();
  ...
  ShowHide(id, i) {
    let stt = this.listCateProduct[i].IsShow ? 1 : 10;
