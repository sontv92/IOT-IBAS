...
@Component({
  selector: 'app-thongketongkhoiluongbetong',
  templateUrl: './thongketongkhoiluongbetong.component.html',
  styleUrls: ['./thongketongkhoiluongbetong.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class ThongKeTongKhoiLuongBeTongComponent implements OnInit {
  ...
  GetListSlide() {
    ...
    this.listTongKhoiLuongBT = [];
    this.listTongDonHangChiTiet = [];

    if (this.paging.CompanyId == undefined) {
      this.paging.CompanyId = 0;
    }
    this.http.get('/api/thongketongkhoiluongbetong/GetByPage?page=' + this.paging.page + '&page_size=' + this.paging.page_size + '&order_by=' + this.paging.order_by 
      ...
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          if(res["data"] != null){
            this.listTongKhoiLuongBT = res["data"];
            this.paging.item_count = res["metadata"];
            switch(this.q.GroupBy){
              case "CT":
                this.listTongKhoiLuongBT.forEach(item => {
            ...
            default:
                  this.listTongKhoiLuongBT.forEach(item => {
                    item.Key = "Ngày lập: " + this.datepipe.transform(new Date(item.Key), 'dd/MM/yyyy');
                  });
                break;
            }
          console.log('listTongKhoiLuongBT',this.listTongKhoiLuongBT);
  ...
  //  debugger;
    fetch('/api/thongketongkhoiluongbetong/GetReportChiTiet?page=' + this.paging.page + '&page_size=' + this.paging.page_size + '&order_by=' + this.paging.order_by 
                                                          + '&tungay=' + _tungay 
                                                          + '&denngay=' + _denngay
                                                          + '&timetungay=' + this.q.timetungay
                                                          + '&timedenngay=' + this.q.timedenngay 
      ...
      .then(function (response) {
        ...
        this.DownloadFile(blob, this.commonService.ConvertUrl(tenchinhanh.Name) + "_thong_ke_tong_khoi_luong_be_tong" + date + ".xlsx", 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
