...
@Component({
  selector: 'app-thongkechitietkhoiluongbetong',
  templateUrl: './thongkechitietkhoiluongbetong.component.html',
  styleUrls: ['./thongkechitietkhoiluongbetong.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class ThongKeChiTietKhoiLuongBeTongComponent implements OnInit {
  ...
  public tablesilde = [];
  public listChiTietKhoiLuongBT = [];
  public listTongDonHangChiTiet = [];
  public role: number;
  public disable:boolean = true;
  public loading :boolean;
  public listMaPhieu = [];
  public listCONGTRUONG = [];
  public listTAIXE = [];
  public listGroup = [ {value: "NL", name: "Ngày lập"},
                       {value: "CT", name: "Công trường"},
  ...
  changeFn(val){
    ...
    this.GetlistCD();
    this.GetlistTenTaiXe(val);
    this.GetlistMaPhieu(val);
  }
  ...
  GetListSlide() {
    ...
    this.listChiTietKhoiLuongBT = [];
    this.listTongDonHangChiTiet = [];

    if (this.paging.CompanyId == undefined) {
      this.paging.CompanyId = 0;
    }
    this.http.get('/api/thongkechitietkhoiluongbetong/GetByPage?page=' + this.paging.page + '&page_size=' + this.paging.page_size + '&order_by=' + this.paging.order_by 
                                                          ...
                                                          + '&TENDUAN=' + this.q.TENDUAN
                                                          + '&MAPHIEU=' + this.q.MAPHIEU
                                                          + '&TAIXE=' + this.q.TAIXE
                                                          + '&CHEDO=' + this.q.CHEDO
      ...
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          if(res["data"] != null){
            this.listChiTietKhoiLuongBT = res["data"];
            this.paging.item_count = res["metadata"];
            switch(this.q.GroupBy){
              case "CT":
                this.listChiTietKhoiLuongBT.forEach(item => {
                  item.Key = "Công trường: " + item.Key;
                });
                break;
              case "KH":
                  this.listChiTietKhoiLuongBT.forEach(item => {
              ...
              case "NV":
                  this.listChiTietKhoiLuongBT.forEach(item => {
                    item.Key = "Nhân viên: " + item.Key;
                  });
                break;
              case "MA":
                  this.listChiTietKhoiLuongBT.forEach(item => {
                    item.Key = "Mác: " + item.Key;
                  });
                break;
            default:
  ...
  ExportExcel() {
    ...
    {
      this.paging.query += " AND [F].TENMACBETONG='"+this.q.TENMACBETONG+"'";
    }
    if (this.q.TENNV != undefined && this.q.TENNV != '')
    {
      this.paging.query += " AND [G].TENNV='"+this.q.TENNV+"'";
    }
    if (this.q.TENHANGMUC != undefined && this.q.TENHANGMUC != '')
    {
      this.paging.query += " AND [H].TENHANGMUC='"+this.q.TENHANGMUC+"'";
    }
    if (this.q.MAPHIEU != undefined && this.q.MAPHIEU != '')
    {
      this.paging.query += " AND [I].MAPHIEU='"+this.q.MAPHIEU+"'";
    }
    if (this.q.TENDUAN != undefined && this.q.TENDUAN != '')
    {
      this.paging.query += " AND [K].TENDUAN='"+this.q.TENDUAN+"'";
    }
    if (this.q.CHEDO != undefined && this.q.CHEDO != '')
    {
      this.paging.query += " AND [L].CHEDO='"+this.q.CHEDO+"'";
    }
    if (this.q.TAIXE != undefined && this.q.TAIXE != '')
    {
      this.paging.query += " AND [M].TAIXE='"+this.q.TAIXE+"'";
    }
    if (this.q.GroupBy != undefined && this.q.GroupBy != '')
    {
      this.paging.query += " AND [M].GroupBy='"+this.q.GroupBy+"'";
    }

    let tenchinhanh = this.listBranchSearch.find(x => x.BranchId == this.paging.Branchid);
  //  debugger;
    fetch('/api/thongkechitietkhoiluongbetong/GetReportChiTiet?page=' + this.paging.page + '&page_size=' + this.paging.page_size + '&order_by=' + this.paging.order_by 
                                                          + '&tungay=' + _tungay 
                                                          + '&denngay=' + _denngay
                                                          + '&timetungay=' + this.q.timetungay
                                                          + '&timedenngay=' + this.q.timedenngay 
      ...
      .then(function (response) {
        ...
        this.DownloadFile(blob, this.commonService.ConvertUrl(tenchinhanh.Name) + "_thong_ke_chi_tiet_khoi_luong_be_tong_" + date + ".xlsx", 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  ...
  GetListDuAn(val) {
    this.http.get('/api/duan/'+ val ,this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listCONGTRUONG = res["data"];
        }
      },
      (err) => {
        console.log("Error: connect to API");
  ...
  //Get danh sách typeAttributeItem
  GetlistTenTaiXe(val) {
    this.http.get('/api/thongkechitietkhoiluongbetong/GetTenTaiXe/'+val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listTAIXE = res["data"];
  ...
  GetlistMaPhieu(val) {
    this.http.get('/api/thongkechitietkhoiluongbetong/GetMaPhieu/'+val, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listMaPhieu = res["data"];
  ...
  calculateCustomerTotal(name) {
    ...
    if (this.listChiTietKhoiLuongBT) {
        for (let item of this.listChiTietKhoiLuongBT) {
            if (item.NGAYTRON === name) {
                total++;
            }
        }
    }
