...
@Component({
  selector: 'app-thongketongvattu',
  templateUrl: './thongketongvattu.component.html',
...
export class ThongKeTongVatTuComponent implements OnInit {
  ...
  public tablesilde = [];
  public listTongVatTuChiTiet = [];
  public listTongVatTu = [];
  public role: number;
  ...
  GetListSlide() {
    ...
    this.listTongVatTu = [];
    this.listTongVatTuChiTiet = [];

    if (this.paging.CompanyId == undefined) {
      this.paging.CompanyId = 0;
    }
    this.http.get('/api/thongketongvattu/GetByPage?page=' + this.paging.page 
    ...
    + '&CVL=' + this.q.CVL + '&CHEDO=' + this.q.CHEDO + '&companyid=' + this.paging.CompanyId + '&Branchlist=' + this.paging.Branchid, this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
          this.listTongVatTuChiTiet = res["data"];
          this.listTongVatTu = res["data1"];
          this.paging.item_count = res["metadata"];

          this.tableMockData = this.listTongVatTuChiTiet;
          this.tableMockDataTong = this.listTongVatTu;
  ...
  ExportExcel() {
    ...
    if (this.q.CVL != undefined && this.q.CVL != '')
    {
      this.paging.query += " AND [D].CVL='"+this.q.CVL+"'";
    }
  ...
  //  debugger;
    fetch('/api/thongketongvattu/GetReportChiTiet?page=' + this.paging.page 
    + '&page_size=' + this.paging.page_size 
    + '&order_by=' + this.paging.order_by 
    + '&tungay=' + this.q.tungay 
    + '&denngay=' + this.q.denngay
    + '&timetungay=' + this.q.timetungay
    + '&timedenngay=' + this.q.timedenngay
    + '&CVL=' + this.q.CVL + '&CHEDO=' + this.q.CHEDO + '&companyid=' + this.paging.CompanyId + '&Branchlist=' + this.paging.Branchid, {
      method: 'GET',
      headers: new Headers({
        'Authorization': 'bearer ' + localStorage.getItem("access_token")
      }),
      ...
      .then(function (response) {
        ...
        this.DownloadFile(blob, this.commonService.ConvertUrl(tenchinhanh.Name) + "_thong_ke_tong_vat_tu_" + date + ".xlsx", 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
  ...
  //Get danh sách cửa vật liệu
  GetListTenCuaVL(val) {
    this.http.get('/api/cuavl/'+ val ,this.httpOptions).subscribe(
      (res) => {
        if (res["meta"]["error_code"] == 200) {
